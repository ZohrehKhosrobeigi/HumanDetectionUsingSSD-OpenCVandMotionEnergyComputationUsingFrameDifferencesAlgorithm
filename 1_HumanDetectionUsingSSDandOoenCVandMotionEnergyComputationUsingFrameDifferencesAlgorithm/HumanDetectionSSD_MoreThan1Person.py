...
class HumanDetection():
    ...
    def humanDetection_Box(self, frame,frame_n,report_folder):
        ...
        for i in range(detections.shape[2]):
           z.append(detections[0, 0, i, 2])
        for i in range (detections.shape[2]):# loop, iterates over the detected objects in a frame processed by the SSD (Single Shot MultiBox Detector) model. gives the number of detections made by the SSD model in the current frame. The loop iterates over each detection to process them individually.
            class_id = int (detections[0, 0, i, 1])
            confidence = detections[0, 0, i, 2]
            wf_report.write("*********** i is: "+str(i)+"\n")
            if confidence > 0.88 and class_id == person_class_ID:
                ...
                label = f"Person {detection_id}: {confidence * 100:.2f}%"
